...
type LogoProps = {
  ...
  className?: string
  iconOnly?: boolean
  /** The "LUXURY CHAUFFEURED TRANSPORTATION" strapline — too small to read in the header. */
  tagline?: boolean
}

export function Logo({ compact = false, className = '', iconOnly = false, tagline = true }: LogoProps) {
  return (
    <Link to="/" className={`group flex items-center gap-2 md:gap-3 ${className}`}>
      ...
      {!iconOnly && (
        <div className={`flex flex-col ${compact ? 'hidden md:flex' : 'flex'}`}>
          <p className="font-display text-base font-bold leading-tight tracking-[0.24em] text-white sm:text-lg md:tracking-[0.28em]">
            DENVER
          </p>
          <p className="font-display text-xs font-semibold leading-tight tracking-[0.14em] text-brand-gold-light sm:text-sm md:tracking-[0.16em]">
            BLACK LIMO <span className="text-[10px] sm:text-[11px]">LLC</span>
          </p>
          {tagline && (
