 ...
 *
 * The client's own wording, transcribed from their signed-off document. It is
 * legal text, so it is kept verbatim rather than polished. Two renderers read
 * it: the booking emails (emails.js) and the website's /terms page, which
 * carries an identical copy in denverblacklimo-frontend/src/content/terms.ts.
 * Edit both together.
 *
 * Block shapes, so each renderer can format without parsing prose:
 *   { type: 'p',       text }                 paragraph
 *   { type: 'sub',     title, text }          inline heading + paragraph
 *   { type: 'bullets', items: [text] }
 *   { type: 'table',   head: [a, b], rows: [[a, b]] }
 */

const COMPANY = 'Denver Black Limo, LLC';
const DISPATCH = '+1 (720) 499-6744';

/** When the full balance is due, by vehicle class. Reused in the emails' highlight box. */
const PAYMENT_SCHEDULE = [
  ['Sedan / SUV', 'Time of booking'],
  ['Executive Van / Limos', '7 days prior to service'],
  ['Coach and Charter', '14 days prior to service'],
];

/** Notice required to cancel without charge, by service. Reused in the emails' highlight box. */
const CANCELLATION_WINDOWS = [
  ['Airport services', '24 hours'],
  ['Sedan / SUV (all other trips)', '72 hours'],
  ['Executive Van / Limos', '7 days'],
  ['Coach and Charter', '14 days'],
];
...
const DAMAGE_FEES = [
  ...
  ['Broken glassware', '$10 per glass/flute, $50 per decanter'],
...
const SECTIONS = [
  {
    ...
    blocks: [
      { type: 'p', text: 'All reservations require immediate payment and/or deposit at the time of booking to secure your reservation. Remainder balances are due following the schedule below. Any changes or additional charges will be processed after the completion of the reservation.' },
      { type: 'table', head: ['Service type', 'Full payment due'], rows: PAYMENT_SCHEDULE },
    ],
  },
  {
    id: 'arrival',
  ...
  },
  {
    ...
    title: 'Rate',
    blocks: [
      { type: 'sub', title: 'All-inclusive', text: 'The rate you are quoted shall remain the same rate billed except for additional time used and possible additional charges listed below.' },
      { type: 'sub', title: 'Gratuity', text: `Chauffeurs are paid an appropriate wage and 100% of the gratuity for the service(s) they perform. If a chauffeur asks for or says they do not get gratuity, please call our office immediately at ${DISPATCH}. Additional gratuities for exceptional services are always welcome at the customer's discretion but are not expected.` },
      { type: 'sub', title: 'Additional charges', text: 'Additional charges may apply for wait times, additional stops, tolls/parking, and damages.' },
    ],
  },
  {
    id: 'airport-policies',
  ...
  },
  {
    ...
    blocks: [
      { type: 'p', text: `Shortly after the plane arrives at the gate, please expect a text from the chauffeur providing you with direction to our designated pick-up location at the airport. In the event the passenger is unable to contact or locate the chauffeur, immediately call dispatch at ${DISPATCH}.` },
      { type: 'sub', title: 'DEN (Curbside)', text: `Once contact between passenger(s) and chauffeur is made. Upon exiting the plane, please follow signs to Main Terminal and Baggage Claim. Please note that this may require you to take a tram to Main Terminal and Baggage Claim. Chauffeur(s) stage outside of baggage claim 16 West Terminal Door 506 island 2 with a sign with the passenger's name on it or outside of baggage claim 6 East Terminal Door 511 island 2 with a sign with the passenger's name on it. In the event the passenger is unable to locate the chauffeur, immediately call dispatch at ${DISPATCH}.` },
      { type: 'sub', title: 'DEN (Inside Meet and Greet)', text: `This service has an additional charge. Upon landing, the passenger should follow the airport signs to Main Terminal and Baggage Claim. This process may require passenger(s) to take the train. After departing the train at the main terminal, the passenger will follow the escalators/elevators up and depart the secure area. Directly outside the secure area, a greeter will be holding a name sign for the passenger. The greeter will escort the passenger through the airport, to baggage claim, and to their chauffeur. In the event the passenger is unable to locate the greeter, immediately call dispatch at ${DISPATCH}.` },
      { type: 'sub', title: 'COS', text: `Upon landing, the passenger should follow airport signage to baggage claim. The chauffeur will be standing at the bottom of the escalators, next to baggage claim, with a name sign. In the event the passenger is unable to contact and locate the chauffeur, immediately call dispatch at ${DISPATCH}.` },
      { type: 'sub', title: 'Other airports', text: `Please note that each airport is different. Passenger(s) pick-up locations may change; however, our procedure does not. Upon arriving at the gate, expect a text from the chauffeur providing a designated pick-up location. This could be inside at baggage claim or curbside, airport permitting. In the event the passenger is unable to contact and locate the chauffeur, immediately call dispatch at ${DISPATCH}.` },
      { type: 'sub', title: "FBO's", text: 'Chauffeur(s) will pick up passenger(s) on the tarmac plane-side at all fixed-base operators whenever available. Please note that certain FBOs have regulations regarding vehicle entrance onto the tarmac that Denver Black Limo, LLC has no control over. Chauffeurs will follow the pick-up procedure provided to them by the specific FBO.' },
    ],
  },
  {
    id: 'deposits',
  ...
  },
  {
    ...
    title: 'Damage Policy',
    blocks: [
      { type: 'p', text: 'By receipt of this confirmation, you agree to be liable for any damages you (the client) or any passenger(s) make to any vehicle during charter, including but not limited to smoking in the vehicles, vomiting in vehicle, excessive spills/messes, burns, tears, broken glassware, etc. In the event of damages, you agree to pay the following charges associated with such damages:' },
      { type: 'table', head: ['Damage', 'Charge'], rows: DAMAGE_FEES },
    ],
  },
  {
    id: 'conduct',
...
module.exports = { COMPANY, DISPATCH, PAYMENT_SCHEDULE, CANCELLATION_WINDOWS, DAMAGE_FEES, PREAMBLE, SECTIONS };
